...
public class BookingRepository : IBookingRepository
{
    ...
    /// <summary>
    /// Load tối thiểu: User + Status + Promotion.
    /// Dùng cho các thao tác chỉ cần kiểm tra quyền / trạng thái (Cancel, Reschedule…).
    /// </summary>
    ...
    /// <summary>
    /// Load đầy đủ navigation cho mapper (BookingDetails, Services, Deposit…).
    /// Dùng khi cần trả về BookingResponse chi tiết.
    /// </summary>
    ...
    {
        // Include tối thiểu cho summary — không load Details/Services/Deposit
        // để tránh cartesian explosion khi count + filter
        ...
        // Filter theo ngày slot: dùng subquery thay vì Include để tránh EF10622
        if (dateFrom.HasValue)
            query = query.Where(b => b.BookingDetails
                .Any(d => d.FieldSlot.SlotDate >= dateFrom.Value));

        if (dateTo.HasValue)
            query = query.Where(b => b.BookingDetails
                .Any(d => d.FieldSlot.SlotDate <= dateTo.Value));

        if (fieldId.HasValue)
            query = query.Where(b => b.BookingDetails
                .Any(d => d.FieldSlot.FieldId == fieldId.Value));
        ...
        var items = await query
            ...
            .Take(pageSize)
            // Load thêm BookingDetails sau khi đã filter + page
            // để MapSummary lấy được EarliestSlot và FieldName
            .Include(b => b.BookingDetails)
