...
public class FieldRepository : IFieldRepository
{
        ...
        string? search, int? typeId, int? statusId, int page, int pageSize)
    {
        ...
        {
            var s = search.Trim();
            // SQL Server mặc định case-insensitive, không cần ToLower()
    ...
    public async Task<Field> CreateAsync(Field field)
    {
        ...
        // Load navigation sau khi có FieldId để mapper dùng được
    ...
    {
        // Entity đã được tracked từ GetByIdAsync nên chỉ cần SaveChanges,
        // không cần gọi .Update() (sẽ mark toàn bộ properties là modified)
